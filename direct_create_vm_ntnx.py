...
filepath = "%s/myscript_msg" %my_dir
print(filepath)
...
def main(task, vm_name, image_name, vm_cluster, vm_network, vm_memory, vm_num_sockets, cores_per_socket, vm_power_state,
         user, password, cvm_address):
    retry_count = 100
    ...
    (status, result) = clone_vm_from_image(restApi=rest_api, vm_name=vm_name, image_uuid=image_uuid, subnet_uuid=subnet_uuid,
                        cluster_uuid=cluster_uuid, memory=vm_memory, num_sockets=vm_num_sockets,
                        cores_per_socket=cores_per_socket, power_state=vm_power_state)
    api_library = ApiLibrary()
    api_response = api_library.cluster_responses
    if api_response[str(status)] == "success":
        ...
        if exe_status.lower() != 'error':
            logging.debug('status is %s' % exe_status)
            print(item_uuid)
            count = 0
            while count < retry_count:
                count += 1
                ...
                print(vm_ip)
                if vm_ip:
                    #vm_ip = result['status']['resources']['nic_list'][0]['ip_endpoint_list'][0].get('ip')
                    #vm_ip = extract_value(result, 'ip')
                    print(vm_ip)
                    break
                #vm_ip = result['status']['resources']['nic_list'][0]['ip_endpoint_list']
                #if vm_ip:
                #    print('exit the loop')
                #    break
                #vm_ip = result['status']['resources']['nic_list'][0]['ip_endpoint_list']
            ...
            print(vm_ip)
            logging.debug('status is %s' % vm_ip)
        else:
            logging.debug('status is %s' % exe_status)
            output = result['status']['message_list'][0]['message']
            logging.debug(output)
            #print(result['status']['message_list'][0]['message'])
...
if __name__ == '__main__':
    ...
    main('clone', vm_name=vm_name, image_name=image_name, vm_cluster=vm_cluster, vm_network=vm_network,
         vm_memory=vm_memory, vm_num_sockets=vm_num_sockets, cores_per_socket=cores_per_socket,
         vm_power_state=vm_power_state, cvm_address=cvm_address, user=user, password=password)
    ###
